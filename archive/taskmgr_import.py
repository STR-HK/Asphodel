...
def read_metadata(readerPath, firstLevelFile, folderInherit=False, folderPath=None):
    ...
    creationtime_t = int(time.timestamp() * 1000)
        ...
        for track in fileinfo.tracks:
            if track.track_type == "Video":
                ...
                if not os.path.isfile(MyThumbnail):
                    if (
                        os.path.isdir("{}{}".format(thumbnailPath, folderPath)) == False
                        and folderInherit == True
                    ):
                        os.makedirs("{}{}".format(thumbnailPath, folderPath))
                    clip = VideoFileClip(fullpath)
                    try:
                        clip.save_frame(MyThumbnail, t=30)
                    except:
                        clip.save_frame(MyThumbnail, t=10)
...
def recursion_metatdata(fullpath, firstLevelFile, newchilden):
    for f in os.listdir(fullpath):
        ...
        r = read_metadata(
            readerPath, realfolderfile, folderInherit=True, folderPath=firstLevelFile
        )
        insert(r)
...
async def taskmgr_import():
    return {"detail": create_file_metadata()}
...
def get_name_metadata(dataset):
    ...
    for data in dataset:
        if data["extension"] != None:
            names.append("{}.{}".format(data["name"], data["extension"]))
        else:
            names.append(data["name"])
    return names


from modules.sqlite3_taskmgr import taskmgr_insert_value as parser


def insert_to_sqlDB(dataset):
    for data in dataset:
        parser.taskmgr_insert_value(
            path=data["path"],
            creationtime=data["creationtime"],
            creationtime_t=data["creationtime_t"],
            name=data["name"],
            size=data["size"],
            bytesize=data["bytesize"],
            length=data["length"],
            lengthIso=data["lengthIso"],
            type_=data["type"],
