...
async def torrent_search(query: str):
    if ( query == None or query == '' ): return {'response':'No query'}
    return {'response':limetorrent_simple_search(query)}
...
def limetorrent_simple_search(query):
    ...
    for tr in trs:
        try:
            ...
            link = 'https://www.limetorrents.pro{}'.format(tr.find('div', class_='tt-name').find_all('a')[-1]['href'])
            ...
            returns.append({'name':name, 'link':link, 'added':added, 'size':size, 'seed':seed, 'leech':leech, 'health':health})
        except:
            continue
    if returns == []:
        return 'No Result'
